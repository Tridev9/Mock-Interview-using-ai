...
const FormMockInterview = ({ initialData }: FormMockInterviewProps) => {
    ...
    const generateAiResponse = async (data: FormData) => {
        const prompt = `
            As an experienced prompt engineer, generate a JSON array containing 10 technical interview questions along with detailed answers based on the following job information. Each object in the array should have the fields "question" and "answer", formatted as follows:
            [
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="w-full p-8 rounded-lg flex flex-col items-start justify-start gap-6 shadow-md ">
                    <FormField control={form.control} name="position" render={({ field }) => (
                        <FormItem className="w-full space-y-4 ">
                            ...
                            <FormControl>
                                <Input {...field} disabled={loading} className="h-12" placeholder="eg:- Full stack developer." value={field.value || ""} />
                            </FormControl>
                        </FormItem>
                    )} />
                    ...
                    <FormField control={form.control} name="description" render={({ field }) => (
                        <FormItem className="w-full space-y-4 ">
                            ...
                            <FormControl>
                                <Textarea {...field} disabled={loading} className="h-12" placeholder="eg:- describe your job role or position....(min 10 char)" value={field.value || ""} />
                            </FormControl>
                        </FormItem>
                    )} />
                    ...
                    <FormField control={form.control} name="experience" render={({ field }) => (
                        <FormItem className="w-full space-y-4 ">
                            ...
                            <FormControl>
                                <Input {...field} type="number" disabled={loading} className="h-12" placeholder="eg:- 5 yrs in number" value={field.value || ""} />
                            </FormControl>
                        </FormItem>
                    )} />
                    ...
                    <div className="w-full flex items-center justify-end gap-6">
                        <Button type="button" size={"sm"} variant={"outline"} disabled={isSubmitting || loading} onClick={onReset}>
                            Reset
                        </Button>
